  ...
  async beforeRegisterNodeDef(nodeType, nodeData) {
    if (nodeData?.name === 'SplineEditor') {
      chainCallback(nodeType.prototype, 'onNodeCreated', function () {
        ...
        this.splineEditor = this.addDOMWidget(
          nodeData.name,
          'SplineEditorWidget',
          element,
          {
        ...
        chainCallback(this, 'onConfigure', function () {
          try {
            ...
            console.error(
              'An error occurred while configuring the editor:',
              error
            )
          }
        })
...
class SplineEditor {
  constructor(context, reset = false) {
    ...
    this.interpolationWidget = context.widgets.find(
      (w) => w.name === 'interpolation'
    )
    this.pointsWidget = context.widgets.find(
      (w) => w.name === 'points_to_sample'
    )
    this.pointsStoreWidget = context.widgets.find(
    ...
    this.samplingMethodWidget = context.widgets.find(
      (w) => w.name === 'sampling_method'
    )
    ...
    if (!reset && this.pointsStoreWidget.value != '') {
      try {
        ...
        if (
          ...
          parsedData.length > 0 &&
          parsedData[0].hasOwnProperty('points')
        ) {
    ...
    this.vis = new pv.Panel()
      ...
      .event('mousedown', function () {
        if (pv.event.shiftKey) {
          // Use pv.event to access the event object
          let scaledMouse = {
          ...
          let { point1Index, point2Index } = self.findClosestPoints(
            self.splines[self.activeSplineIndex].points,
            clickedPoint
          )
    ...
    this.vis
      ...
      .data(() => {
        ...
        if (
          activeSpline.isSinglePoint ||
          (activeSpline.points && activeSpline.points.length === 1)
        ) {
      ...
      .event('drag', function () {
        ...
        self.splines[self.activeSplineIndex].points[this.index] = {
          x: adjustedX,
          y: adjustedY
      ...
      .text((d) => {
        if (this.samplingMethod == 'path') {
          ...
          let normalizedY =
            (1.0 - d.y / self.height - 0.0) * (self.rangeMax - self.rangeMin) +
            self.rangeMin
    ...
    this.vis
      ...
      .shape('square')
      .strokeStyle((d) =>
        d.splineIndex === this.activeSplineIndex ? '#ff7f0e' : d.color
      )
  ...
  updatePath = () => {
    ...
    if (this.samplingMethod != 'controlpoints') {
      coords = this.samplePoints(
        this.pathElements[this.activeSplineIndex],
        this.points_to_sample,
        this.samplingMethod,
        this.width,
        this.activeSplineIndex
      )
    } else {
      coords = activePoints
    }
    ...
    for (let i = 0; i < this.splines.length; i++) {
      ...
      if (this.samplingMethod != 'controlpoints' && pathElement) {
        splineCoords = this.samplePoints(
          pathElement,
          this.points_to_sample,
          this.samplingMethod,
          this.width,
    ...
    this.backgroundImage
      .url(
        file
          ...
          : `data:${this.node.properties.imgData.type};base64,${base64String}`
      )
      .visible(true)
      .root.render()
  }
  ...
  processImage = (img, file) => {
    ...
    const base64String = canvas
      ...
      .replace('data:', '')
      .replace(/^.+,/, '') // 0.5 is the quality from 0 to 1
  ...
  refreshSplineElements = () => {
    ...
    const linesToRemove = originalChildren.filter(
      (child) => child instanceof pv.Line
    )
  ...
  createContextMenu = () => {
    ...
    document.addEventListener('click', function (e) {
      document
        .querySelectorAll('.spline-editor-context-menu')
        .forEach((menu) => {
        ...
        switch (id) {
          case 0:
            ...
            if (!self.drawHandles) {
              ...
              self.vis
                .add(pv.Line)
                .data(() =>
                  self.splines[self.activeSplineIndex].points.map(
                    (point, index) => ({
                      start: point,
                      end: [index]
                    })
          ...
          case 8: // Delete current spline
            if (self.splines.length > 1) {
              ...
              self.activeSplineIndex = Math.min(
                self.activeSplineIndex,
                self.splines.length - 1
              )
              self.refreshSplineElements()
            }
            self.node.contextMenu.style.display = 'none'
          ...
          case 9: // Next spline
            self.activeSplineIndex =
              (self.activeSplineIndex + 1) % self.splines.length
            self.refreshSplineElements()
            self.node.contextMenu.style.display = 'none'
            break
        }
  ...
  samplePoints(svgPathElement, numSamples, samplingMethod, width, splineIndex) {
    ...
    if (
      spline &&
      (spline.isSinglePoint || (spline.points && spline.points.length === 1))
    ) {
    ...
    if (!svgPathElement) {
      console.warn(
        `Path element not found for spline index: ${splineIndex}. Available paths: ${this.pathElements.length}`
      )
      ...
      for (let i = 0; i < numSamples; i++) {
        ...
        const x =
          splinePoints[idx].x +
          fraction * (splinePoints[idx + 1].x - splinePoints[idx].x)
        const y =
          splinePoints[idx].y +
          fraction * (splinePoints[idx + 1].y - splinePoints[idx].y)
    ...
    if (samplingMethod === 'speed') {
      ...
      for (const cp of controlPoints) {
        ...
        for (let pos = 0; pos <= pathLength; pos += pathLength / 100) {
          ...
          const dist = Math.sqrt(
            Math.pow(pt.x - cp.x, 2) + Math.pow(pt.y - cp.y, 2)
          )
      ...
      const createSynchronizedMapping = () => {
        ...
        const segments = []
        let totalLength =
          pathPositions[pathPositions.length - 1] - pathPositions[0]
        ...
        return (t) => {
          ...
          const segmentStart =
            segmentIndex > 0 ? cumulativeWeights[segmentIndex - 1] : 0
  ...
  findClosestPoints(points, clickedPoint) {
    ...
    if (closestPoint1Index > closestPoint2Index) {
      ;[closestPoint1Index, closestPoint2Index] = [
        closestPoint2Index,
        closestPoint1Index
      ]
    }
