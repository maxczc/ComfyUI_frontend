  ...
  async beforeRegisterNodeDef(nodeType, nodeData) {
    if (nodeData?.name === 'PointsEditor') {
      chainCallback(nodeType.prototype, 'onNodeCreated', function () {
        hideWidgetForGood(
          this,
          this.widgets.find((w) => w.name === 'coordinates')
        )
        hideWidgetForGood(
          this,
          this.widgets.find((w) => w.name === 'neg_coordinates')
        )
        hideWidgetForGood(
          this,
          this.widgets.find((w) => w.name === 'bboxes')
        )
        ...
        this.pointsEditor = this.addDOMWidget(
          nodeData.name,
          'PointsEditorWidget',
          element,
          {
...
class PointsEditor {
  constructor(context, reset = false) {
    ...
    this.neg_coordWidget = context.widgets.find(
      (w) => w.name === 'neg_coordinates'
    )
    ...
    this.vis = new pv.Panel()
      ...
      .event('mousedown', function () {
        if (pv.event.shiftKey && pv.event.button === 2) {
          ...
          console.log(
            'start drawing at ' +
              this.mouse().x / app.canvas.ds.scale +
              ', ' +
              this.mouse().y / app.canvas.ds.scale
          )
          drawing = true
    ...
    //create bounding box
    this.bounding_box = this.vis
      .add(pv.Area)
      .data(function () {
        if (
          drawing ||
          (self.bbox && self.bbox[0] && Object.keys(self.bbox[0]).length > 0)
        ) {
      ...
      .bottom(function () {
        return self.height - Math.max(self.bbox[0].startY, self.bbox[0].endY)
      })
      .left(function (d) {
        return d
      })
      .height(function () {
        return Math.abs(self.bbox[0].startY - self.bbox[0].endY)
      })
      .fillStyle('rgba(70, 130, 180, 0.5)')
      .strokeStyle('steelblue')
      .visible(function () {
        return drawing || Object.keys(self.bbox[0]).length > 0
      })
      .add(pv.Dot)
      .visible(function () {
        return drawing || Object.keys(self.bbox[0]).length > 0
    ...
    this.vis
      ...
      .strokeStyle(function () {
        return i == this.index ? '#07f907' : '#139613'
      })
      .lineWidth(4)
      .fillStyle(function () {
        return 'rgba(100, 100, 100, 0.6)'
    ...
    this.vis
      ...
      .cursor('move')
      .strokeStyle(function () {
        return i == this.index ? '#f91111' : '#891616'
      })
      .lineWidth(4)
      .fillStyle(function () {
        return 'rgba(100, 100, 100, 0.6)'
      ...
      .font(25 + 'px sans-serif')
      .text((d) => {
        return this.neg_points.indexOf(d)
      })
      .textStyle('red')
      .textShadow('2px 2px 2px black')
      .add(pv.Dot) // Add smaller point in the center
