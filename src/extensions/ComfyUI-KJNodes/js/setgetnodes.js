    ...
    class SetNode extends LGraphNode {
      ...
      constructor(title) {
        ...
        this.addWidget(
          ...
          (s, t, u, v, x) => {
            ...
            if (this.widgets[0].value !== '') {
              this.title =
                (!disablePrefix ? 'Set_' : '') + this.widgets[0].value
            }
            this.update()
        ...
        this.update = function () {
          ...
          const allGetters = node.graph._nodes.filter(
      ...
      getExtraMenuOptions(_, options) {
        this.menuEntry = this.drawConnection
          ? 'Hide connections'
          : 'Show connections'
        options.unshift(
          {
            ...
            callback: () => {
              ...
              this.slotColor =
                this.canvas.default_connection_color_byType[linkType]
              this.menuEntry = this.drawConnection
                ? 'Hide connections'
                : 'Show connections'
          ...
          },
          {
            ...
            callback: () => {
              const allGetters = this.graph._nodes.filter(
                (otherNode) =>
                  otherNode.type === 'GetNode' || otherNode.type === 'SetNode'
              )
              allGetters.forEach((otherNode) => {
      ...
      _drawVirtualLinks(lGraphCanvas, ctx) {
        ...
        if (!this.flags.collapsed) {
          ...
          var start_node_slotpos = [title_width + 55, -15]
        }
...
app.registerExtension({
  ...
  registerCustomNodes() {
    class GetNode extends LGraphNode {
      ...
      constructor(title) {
        ...
        this.onRename = function () {
          ...
          if (setter) {
            ...
            this.title =
              (!disablePrefix ? 'Get_' : '') + setter.widgets[0].value
          ...
          if (this.outputs[0].type !== '*' && this.outputs[0].links) {
            this.outputs[0].links
              .filter((linkId) => {
                ...
                return (
                  link &&
                  !link.type.split(',').includes(this.outputs[0].type) &&
                  link.type !== '*'
                )
              })
              .forEach((linkId) => {
      ...
      onAdded(graph) {}
      getExtraMenuOptions(_, options) {
        let menuEntry = this.drawConnection
          ? 'Hide connections'
          : 'Show connections'
