    ...
    switch (nodeData.name) {
          ...
          this.addWidget('button', 'Set all', null, () => {
            ...
            if (userInput) {
              ...
              if (match) {
                ...
                let targetWidgets = []
                if (type === 's') {
                  targetWidgets = this.widgets.filter((widget) =>
                    widget.name.includes('single')
                  )
                } else if (type === 'd') {
                  targetWidgets = this.widgets.filter((widget) =>
                    widget.name.includes('double')
                  )
                } else {
                ...
                alert(
                  'Invalid input format. Please use the format s0,1,2-7=2.0, d0,1,2-7=2.0, or 1.0'
                )
              }
            } else {
      ...
      case 'GetMaskSizeAndCount':
        ...
        nodeType.prototype.onConnectInput = function (
          ...
          const v = onGetMaskSizeConnectInput
            ? onGetMaskSizeConnectInput.apply(this, arguments)
            : undefined
        ...
        nodeType.prototype.onExecuted = function (message) {
          const r = onGetMaskSizeExecuted
            ? onGetMaskSizeExecuted.apply(this, arguments)
            : undefined
      ...
      case 'GetImageSizeAndCount':
        ...
        nodeType.prototype.onConnectInput = function (
          ...
          console.log(this)
          const v = onGetImageSizeConnectInput
            ? onGetImageSizeConnectInput.apply(this, arguments)
            : undefined
          //console.log(this)
          this.outputs[1]['label'] = 'width'
          this.outputs[2]['label'] = 'height'
        ...
        nodeType.prototype.onExecuted = function (message) {
          console.log(this)
          const r = onGetImageSizeExecuted
            ? onGetImageSizeExecuted.apply(this, arguments)
            : undefined
          let values = message['text'].toString().split('x').map(Number)
          console.log(values)
          this.outputs[1]['label'] = values[1] + ' width'
      ...
      case 'GetLatentSizeAndCount':
        ...
        nodeType.prototype.onConnectInput = function (
          ...
          console.log(this)
          const v = onGetLatentConnectInput
            ? onGetLatentConnectInput.apply(this, arguments)
            : undefined
          //console.log(this)
          this.outputs[1]['label'] = 'width'
          this.outputs[2]['label'] = 'height'
        ...
        nodeType.prototype.onExecuted = function (message) {
          console.log(this)
          const r = onGetLatentSizeExecuted
            ? onGetLatentSizeExecuted.apply(this, arguments)
            : undefined
          let values = message['text'].toString().split('x').map(Number)
          console.log(values)
      ...
      case 'PreviewAnimation':
        ...
        nodeType.prototype.onConnectInput = function (
          ...
          const v = onPreviewAnimationConnectInput
            ? onPreviewAnimationConnectInput.apply(this, arguments)
            : undefined
        ...
        nodeType.prototype.onExecuted = function (message) {
          const r = onPreviewAnimationExecuted
            ? onPreviewAnimationExecuted.apply(this, arguments)
            : undefined
      ...
      case 'VRAM_Debug':
        ...
        nodeType.prototype.onConnectInput = function (
          ...
          const v = onVRAM_DebugConnectInput
            ? onVRAM_DebugConnectInput.apply(this, arguments)
            : undefined
        ...
        nodeType.prototype.onExecuted = function (message) {
          const r = onVRAM_DebugExecuted
            ? onVRAM_DebugExecuted.apply(this, arguments)
            : undefined
      ...
      case 'JoinStringMulti':
        ...
        nodeType.prototype.onNodeCreated = function () {
          ...
          this.addWidget('button', 'Update inputs', null, () => {
            ...
            const num_inputs = this.inputs.filter(
              (input) =>
                input.name && input.name.toLowerCase().includes('string_')
            ).length
            if (target_number_of_inputs === num_inputs) return // already set, do nothing
        ...
        nodeType.prototype.onNodeCreated = function () {
          ...
          const updateWidgetValueInRealTime = () => {
            ...
            if (analyser && dataArray) {
              ...
              const startRangeHzWidget = this.widgets.find(
                (w) => w.name === 'start_range_hz'
              )
              if (startRangeHzWidget) startRangeHz = startRangeHzWidget.value
              const endRangeHzWidget = this.widgets.find(
                (w) => w.name === 'end_range_hz'
              )
              if (endRangeHzWidget) endRangeHz = endRangeHzWidget.value
              const smoothingFactorWidget = this.widgets.find(
                (w) => w.name === 'smoothing_factor'
              )
              if (smoothingFactorWidget)
                smoothingFactor = smoothingFactorWidget.value
              ...
              const startRangeIndex = Math.floor(
                startRangeHz / frequencyBinWidth
              )
              ...
              const calculateAverage = (start, end) => {
                ...
                smoothedSoundLevel =
                  average * (1 - smoothingFactor) +
                  smoothedSoundLevel * smoothingFactor
                return smoothedSoundLevel
              }
              // Calculate the average levels for each frequency range
              const soundLevel = calculateAverage(
                startRangeIndex,
                endRangeIndex
              )
              ...
              const lowLevelWidget = this.widgets.find(
                (w) => w.name === 'sound_level'
              )
          ...
          const startMicrophoneCapture = () => {
            ...
            if (!audioContext) {
              ...
              const lowRangeWidget = this.widgets.find(
                (w) => w.name === 'low_range_hz'
              )
              if (lowRangeWidget) startRangeHz = lowRangeWidget.value

              const midRangeWidget = this.widgets.find(
                (w) => w.name === 'mid_range_hz'
              )
