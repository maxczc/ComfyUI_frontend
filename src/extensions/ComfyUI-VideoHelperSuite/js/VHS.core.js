    ...
    if (type == 'folder') {
      Object.assign(fileInput, {
        ...
        onchange: async () => {
          ...
          let successes = 0
          const onProg = (p) =>
            (this.progress = (successes + p) / fileInput.files.length)
  ...
  chainCallback(nodeType.prototype, 'onNodeCreated', function () {
    ...
    previewWidget.updateSource = function () {
      ...
      if (!advp) {
        ...
        element.src = api.apiURL(
          '/vhs/viewaudio?' + new URLSearchParams(params)
        )
      }
    }
      ...
      if (
        ...
        if (!advp) {
          ...
          this.videoEl.src = api.apiURL(
            '/vhs/viewvideo?' + new URLSearchParams(params)
          )
        }
...
function fitText(ctx, text, maxLength) {
  ...
  let shortened = text.slice(0, Math.max(0, cutoff - 2)) + '鈥�'
...
function fitPath(ctx, path, maxLength) {
  ...
  if (filename.length > len - 2) {
    ...
    if (cutoff < 0) {
      ...
      displayPath = (isAbs ? '/鈥�' : '鈥�') + partial.substr(cutoff)
    }
  }
...
function mouseAnnotated(event, [x, y], node) {
  ...
  if (x > margin + 6 && x < margin + 16) {
    ...
    (x > widget_width - margin - 16) &
    (x < widget_width - margin - 6)
  ) {
    isButton = 1
  ...
  if (allow_scroll && event.type == 'pointermove') {
    if (event.deltaX) this.value += event.deltaX * (this.options.step || 1)
  ...
  settings: [
    {
      ...
      category: ['馃帴馃叆馃厳馃參', 'Previews', 'Advanced Previews'],
    ...
    },
    {
      ...
      category: ['馃帴馃叆馃厳馃參', 'Previews', 'Min Width'],
    ...
    },
    {
      ...
      category: ['馃帴馃叆馃厳馃參', 'Previews', 'Deadline'],
      name: 'Deadline',
      tooltip:
    ...
    },
    {
      ...
      category: ['馃帴馃叆馃厳馃參', 'Previews', 'Default Mute'],
    ...
    },
    {
      ...
      category: ['馃帴馃叆馃厳馃參', 'Sampling', 'Latent Previews'],
    ...
    },
    {
      ...
      category: ['馃帴馃叆馃厳馃參', 'Sampling', 'Latent Preview Rate'],
    ...
    },
    {
      ...
      category: ['馃帴馃叆馃厳馃參', 'Output', 'MetadataImage'],
    ...
    },
    {
      ...
      category: ['馃帴馃叆馃厳馃參', 'Output', 'Keep Intermediate'],
...
function beginLatentPreview(id, previewImages, rate) {
  ...
  animateIntervals[id] = setInterval(() => {
    if (
      getNodeById(id)?.progress == undefined ||
    ...
    )?.drawImage?.(previewImages[displayIndex], 0, 0)
...
api.addEventListener(
  ...
  async (e) => {
    ...
    previewImagesDict[id][index] = await window.createImageBitmap(
      e.detail.slice(24)
    )
    return false
  },
